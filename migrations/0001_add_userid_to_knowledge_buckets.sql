...
COMMENT ON COLUMN evidence.user_id IS 'User ID for data isolation - NULL for guest users';
COMMENT ON COLUMN evidence.is_guest IS 'Flag indicating whether this is guest data (should be periodically cleaned)';
COMMENT ON COLUMN knowledge_embeddings.user_id IS 'User ID for data isolation - NULL for guest users';
