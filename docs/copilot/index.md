...
### Layer 1 — Short-Term Memory (file-based, injected every turn)

- **File:** `logs/Short_Term_Memory.md`
- **Manager:** `server/services/prompt-composer.ts`
- **How:** Read on every request, appended to system prompt
- **AI writes to it** via `logs/STM_APPEND.md` (auto-merged and deleted) or the `append` tool
- **Also:** `logs/cache.md` holds thoughts-forward from prior turn

### Layer 2 — Conversation Summaries (DB, for Evolution Engine only)

- **Table:** `conversation_summaries` in SQLite
- **Manager:** `server/services/summarization-engine.ts`
- **How:** Generated by Gemini 2.0 Flash; used by Evolution Engine pattern analysis, NOT injected into prompts

These two systems are **completely separate**. Don't conflate them.
