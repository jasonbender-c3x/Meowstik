...
export async function perplexitySearch(params: PerplexitySearchParams): Promise<{
  ...
  try {
    ...
    console.error('[Perplexity] perplexitySearch error:', error);
    return {
...
export async function perplexityQuickAnswer(query: string): Promise<{
  ...
  try {
    ...
    console.error('[Perplexity] perplexityQuickAnswer error:', error);
    return {
      answer: `Quick answer failed: ${error.message}`,
      citations: []
    };
  }
}
...
- A summary at the end`,
    ...
    console.error('[Perplexity] perplexityDeepResearch error:', error);
    return {
      answer: `Deep research failed: ${error.message}`,
      citations: [],
      usage: { promptTokens: 0, completionTokens: 0 }
    };
  }
...
export async function perplexityNews(query: string, recency: 'hour' | 'day' | 'week' = 'day'): Promise<{
  ...
  try {
    ...
    console.error('[Perplexity] perplexityNews error:', error);
    return {
      answer: `News search failed: ${error.message}`,
      citations: []
    };
  }
}
